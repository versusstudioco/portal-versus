...
const VIEW_META = {
 ...
 edicion: [' Creativa', 'Editar y diseñar: cola por prioridad'],
...
function metricsFromMet(met) {
 ...
 if (m.views) parts.push('Vistas ' + (+m.views).toLocaleString('es-CO'));
 if (m.likes) parts.push('Likes ' + (+m.likes).toLocaleString('es-CO'));
 if (m.comments) parts.push('Coment. ' + (+m.comments).toLocaleString('es-CO'));
 if (m.saved) parts.push('Guard. ' + (+m.saved).toLocaleString('es-CO'));
 if (m.shared) parts.push('Comp. ' + (+m.shared).toLocaleString('es-CO'));
...
async function marcaEstrategia(marca) {
 ...
 <label class="est-card-vis"><input type="checkbox" id="estCardCli" checked> Visible para el cliente <em>(aparece en su calendario)</em></label>
 </div>
 </div>
...
async function estGenerar(marca, kind, btn) {
 ...
 <h3> ${esc(c.angulo || 'Caption ' + (i + 1))} ${sourcePill(data.source)}</h3>
...
function renderLive(area, d, topic, country) {
 ...
 <b>${PLATFORM_LABEL[l.platform]}</b><span>${esc(l.label)} Comp.</span>
 </a>`).join('') + '</div>';
...
function renderRadar(out, d, topic, country) {
 ...
 html += (d.links || []).map(l => `
 <a class="link-btn plat-${l.platform}" href="${esc(l.url)}" target="_blank" rel="noopener">
 <b>${PLATFORM_LABEL[l.platform]}</b><span>${esc(l.label)} Comp.</span></a>`).join('') + '</div>';
...
$('#cGenerate').addEventListener('click', async () => {
 ...
 <div class="result-card">
 <h3> ${esc(c.angulo || 'Caption ' + (i + 1))} ${sourcePill(data.source)} ${c.longitud ? `<span class="tag">${esc(c.longitud)}</span>` : ''}</h3>
...
function renderHashtags(out, data) {
 ...
 <div class="result-card">
 <h3>#⃣ Estrategia ${sourcePill(data.source)} ${data.categoria ? `<span class="tag tag--red">${esc(data.categoria)}</span>` : ''}</h3>
